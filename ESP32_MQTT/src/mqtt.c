...
const char *CLIENT_ID = "test_device";
...
void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        msg_id = esp_mqtt_client_publish(client, "env_data", "{\"State\": 1, \"Count\": 0}", 0, 1, 0);
        ESP_LOGI(TAG_MQTT, "Just Connected, Sent publish successful, msg_id=%d", msg_id);

        // esp_mqtt_client_subscribe(client, "env_data", 2); // 订阅一个/control的topic

        // msg_id = esp_mqtt_client_subscribe(client, "env_data", 1);
